...
## Buzzer Studio (no install — runs in the browser)

A static showcase of the whole pipeline, deployed by `.github/workflows/pages.yml`:
a 36-moment demo **catalogue** (generated by the real scorer, renderer and
content firewall) and a live **designer** where you drag the shot point on a
court and the poster re-renders in real time, with SVG/PNG downloads.

Enable once: **Settings → Pages → Source: Deploy from a branch → `gh-pages`**,
then visit `https://specifxx.github.io/Buzzer/`. Alternatively download the
`buzzer-studio-site` artifact from any `pages` workflow run and open
`index.html` — the site is self-contained and works from disk.

The browser renderer (`site/buzzer-render.js`) is a port of the Python
layout math; CI re-validates its output against the Python content
firewall on every deploy, so the legal rules hold in the browser too.
